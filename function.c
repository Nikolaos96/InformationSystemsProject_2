...
//     printf("r1 %d\nc1 %lu\ncol %d\ntelestis %d\n value %lu \n", r1, c1, col, telestis, value);
 ...
 void  make_second_intermid(info_deikti *join_list, main_pointer *imid_list, int size_imid_list, int rel,int rel2, int join_stil_A, int join_stil_B){
     ...
     if(take_relation(&imid_list[1], l) == join_stil_A)
         join = 0;	// key
     else
         join = 1;	// payload
...
//		 printf("4 stiles endiameso   %d  %d   %d  %d \n", take_relation(&imid_list[0], 0), take_relation(&imid_list[0], 1), take_relation(&imid_list[0], 2), take_relation(&imid_list[0], 3));
